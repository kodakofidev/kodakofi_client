          ...
          <table className="table-auto min-w-5xl select-none w-full border-separate border-spacing-y-3">
            ...
            <tbody>
                {products.map((product, index) => (
                    <RowListProduct id={product.id} description={product.description} method={product.method} price={product.price} productName={product.name} productSize={product.sizes} stock={product.stock} key={index} dispatch={dispatch} modalAction={modalAction}/>
                ))}
            </tbody>
          </table>
        </section>
